...
当用户给出对标视频链接、本地视频、逐字稿或截图时，使用 `bifang-clip` 先按“开头怎么抓人、讲了哪个问题、用了什么场景、中间怎么解释、结尾怎么收”拆出可迁移结构，再替换为客户真实的人群、证据和承接动作。它不做逐字洗稿，也不承诺爆款。

### 老账号规划

当用户已有作品时，使用 `bifang-account-plan`。优先查看播放最高、私信/成交最多、长期无效但常发的三组内容，把内容归类为流量、信任、成交和无效消耗资产；再给未来 30 天方向、内容配比、下一周发布表与复盘规则。老账号不会被重新当成新账号做个人资料建档。

完整的产品边界、输入要求和验收用例见 [BIFANG_V0.3_PRODUCT.md](BIFANG_V0.3_PRODUCT.md) 与 [tests/v0.3_acceptance.md](tests/v0.3_acceptance.md)。
